...
interface SearchProps {
  ...
  loadItems: Dispatch<SetStateAction<CategoriesType[]>> | Dispatch<SetStateAction<TransactionsType[]>>;
...
const SearchBar = ({
  ...
  const endpoint = isTransactionPage ? "accounts" : "category";
  ...
  const searchReceiptCategories = useCallback(async () =>  {
    const id = localStorage.getItem("@finsys:user_id");
  ...
  }, [isCategorySearch, categoryId, searchReceiptCategories]);  
  return(
    <Container isCategorySearch={isCategorySearch}>
      ...
      {isCategorySearch ? (
        <SearchByCategory transactionType={transactionType} setCategoryId={setCategoryId}/>
      ) : (
        <>
          <input 
            value={searchText}
          ...
          />
          <Image src={searchIcon} alt="/" width={12} onClick={searchReceiptCategories} />
      </>
      )}
    </Container>
  );
